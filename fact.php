...
<?php
	...
	$pdf->Cell(25,6,'Color',1,0,'C');
	$pdf->Cell(10,6,'Cant',1,0,'C');
	$pdf->Cell(10,6,'Talla',1,0,'C');
	...
	$pdf->setX($lMargin);
	$pdf->SetWidths(array(30,20,55,25,10,10,15,15));
		...
		if($i<=$vert){
			$j = $j+18;
			$pdf->Row(array($product->barcode,$pdf->Image('storage/products/'.$image1,46,$j+18,18,18),utf8_decode($product->modelo." ".$product->sex),utf8_decode($color->name),$op->q,$size->size,number_format($product->price_out,2,".",","),number_format($op->q*$product->price_out,2,".",",")));
			$i = $i+1;
		}
	}
	$pdf->Ln(6);
