...
class Serie_sizeData {
	...
	public function add(){
		$sql = "insert into serie_size (serie_id,size)";
		$sql .= "value (\"$this->serie_id\",\"$this->size\")";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto Serie_sizeData previamente utilizamos el contexto
	...
	public static function getAllSerieId($id){
		$sql = "select * from ".self::$tablename." where serie_id=$id order by serie_id"; /*order by size_3 desc*/
		$query = Executor::doit($sql);
		return Model::many($query[0],new Serie_sizeData());
	}
